...
function Home() {
  ...
  const handleCheckboxChange = async (noteId, checked) => {
    try {
      ...
      await axios.put(`http://${ip}:8000/api/todos/${noteId}/`, updatedNote, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <>
      <header className="header">
        <button onClick={handleLogout} className="logout-btn">
          <img alt="img" src={log} height="40" width="40" />
        </button>
      ...
      <div className="main-container">
        <div className="notes-container">
          <div className="notes">
            {notes
              .filter((note) =>
                note.title.toLowerCase().includes(search_td.toLowerCase())
              )
              .map((note) => (
                <p className="notes-main" key={note.id}>
                  <input
                    ...
                    checked={note.status}
                    onChange={(e) =>
                      handleCheckboxChange(note.id, e.target.checked)
                    }
                  />
                  ...
                  >
                    <img src={svg} alt="remove" height="30" width="30" />
                  </button>
                </p>
              ))}
          </div>
